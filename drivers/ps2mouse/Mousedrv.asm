...
intro_string	db 'DPSMouseDRV (c) 1998$'
...
_reset_mouse:
	...
	mov	word [cs:x_move],0
	mov	word [cs:y_move],0
	mov	word [cs:x_max],639
	mov	word [cs:x_min],0
	mov	word [cs:y_max],199
	mov	word [cs:y_min],0
	mov	word [cs:user_mask],0
	mov	word [cs:user_subroutine],0
	mov	word [cs:user_subroutine+2],0
	mov	ax,0ffffh
	mov	bx,3
	iret

_get_pos:
	mov	cx,word [cs:pos_x]
	mov	dx,word [cs:pos_y]
	mov	bx,word [cs:buttons]
	xor	bh,bh
	iret

_get_mouse_movement:
	mov	cx,word [cs:x_move]
	mov	dx,word [cs:y_move]
	mov	word [cs:x_move],0
	mov	word [cs:y_move],0
	iret
...
_show_mouse:
	...
	push	di
	push	es
	mov	byte [cs:sm_flag],1
	mov	ax,word [cs:pos_y]
	shr	ax,3
	mov	bl,80
	mul	bl
	mov	bx,word [cs:pos_x]
	shr	bx,3
	add	ax,bx
	shl	ax,1
	mov	di,ax
